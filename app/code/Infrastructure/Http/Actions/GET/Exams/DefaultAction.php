...
namespace Romchik38\Site2\Infrastructure\Http\Actions\GET\Exams;
...
final class DefaultAction extends AbstractMultiLanguageAction implements DefaultActionInterface
{
    ...
    {
        $command = new Find('exams', $this->getLanguage());
        try {
            return $this->pageService->find($command);
        } catch (NoSuchPageException) {
            throw new RuntimeException('Exams view action error: page with url login not found');
        } catch (CouldNotFindException $e) {
            throw new RuntimeException(sprintf('Exams view action error %s: ', $e->getMessage()));
        }
    }
}
